...
// Add Member to a Project project
export const AddMember = async(req, res) => {
    try {
        ...
        if(!project.team_lead !== userId) {
            return res.status(404).json({message: "Only project leads can add members"});
        }
        ...
        if(existingMember){
            return res.status(400).json({message: "User is already a members"});
        }
